...
def createBPFilter(shape, center, bandCenter, bandWidth, lpType=2, n=2):
    ...
    if lpType == 0:  # Ideal bandpass filter
        ...
        lpFilter = 1.0 - 1.0 / (1 + np.power(d * bandWidth / (abs(pow(d, 2) - pow(bandCenter, 2)) + 1), 2 * n))  # plus 1 (because divide by zero error)

    elif lpType == 2:  # Gaussian bandpass filter
        lpFilter = np.exp(
            - pow((pow(d, 2) - pow(bandCenter, 2)) / ((d * bandWidth) + 1), 2))  # plus 1 (because divide by zero error)
...
BpFilter_1 = createBPFilter(img_fft.shape, maxLoc, BC, BW, 0)  # ideal Band-pass filter
BpFilter_2 = createBPFilter(img_fft.shape, maxLoc, BC, BW, 1)  # Butterworth Band-pass filter
BpFilter_3 = createBPFilter(img_fft.shape, maxLoc, BC, BW, 2)  # Gaussian Band pass filter
# --------------------------------------------------------
img_filter1 = BpFilter_1 * img_fft
img_filter2 = BpFilter_2 * img_fft
img_filter3 = BpFilter_3 * img_fft
...
for i in ([[BpFilter_1, 'ideal', gray_spectrum1], [BpFilter_2, 'Butterworth', gray_spectrum2
                                                   ], [BpFilter_3, 'Gaussian', gray_spectrum3]]):
    plt.figure(figsize=(19, 16))
    plt.subplot(121), plt.imshow(cv2.magnitude(i[0][:, :, 0], i[0][:, :, 1]), cmap='gray')
    plt.title('{} band-pass filter'.format(i[1])), plt.xticks([]), plt.yticks([])
...
plt.figure(figsize=(20, 16))
plt.subplot(131), plt.imshow(ori_img1, cmap='gray'), plt.title('image with ideal band-pass filter')
plt.xticks([]), plt.yticks([])
plt.subplot(132), plt.imshow(ori_img2, cmap='gray'), plt.title('image with Butterworth band-pass filter')
plt.xticks([]), plt.yticks([])
plt.subplot(133), plt.imshow(ori_img3, cmap='gray'), plt.title('image with Gaussian  band-pass filter')
plt.xticks([]), plt.yticks([])
plt.show()
