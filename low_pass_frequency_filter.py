...
def createLPFilter(shape, center, radius, lpType=2, n=2):
    ...
    lpFilter_matrix = np.zeros(shape, np.float32)  # shape (rows, cols, 2)
...
lpFilter_1 = createLPFilter(img_fft.shape, maxLoc, radius, 0)  # ideal low-pass filter
lpFilter_2 = createLPFilter(img_fft.shape, maxLoc, radius, 1)  # Butterworth low-pass filter
lpFilter_3 = createLPFilter(img_fft.shape, maxLoc, radius, 2)  # Gaussian low pass filter
# --------------------------------------------------------
img_filter1 = lpFilter_1 * img_fft
img_filter2 = lpFilter_2 * img_fft
img_filter3 = lpFilter_3 * img_fft
...
for i in ([[lpFilter_1, 'ideal', gray_spectrum1], [lpFilter_2, 'Butterworth', gray_spectrum2
                                                   ], [lpFilter_3, 'Gaussian', gray_spectrum3]]):
    plt.figure(figsize=(19, 16))
    plt.subplot(121), plt.imshow(cv2.magnitude(i[0][:, :, 0], i[0][:, :, 1]), cmap='gray')
    plt.title('{} low-pass filter'.format(i[1])), plt.xticks([]), plt.yticks([])
...
plt.figure(figsize=(20, 16))
plt.subplot(131), plt.imshow(ori_img1, cmap='gray'), plt.title('image with ideal low-pass filter')
plt.xticks([]), plt.yticks([])
plt.subplot(132), plt.imshow(ori_img2, cmap='gray'), plt.title('image with Butterworth low-pass filter')
plt.xticks([]), plt.yticks([])
plt.subplot(133), plt.imshow(ori_img3, cmap='gray'), plt.title('image with Gaussian  low-pass filter')
plt.xticks([]), plt.yticks([])
plt.show()
